...
def insert_avl(node, value):
    ...
    if balance > 1 and value < node.left.value:   # Gauche-Gauche
        return rotate_right(node)
    if balance < -1 and value > node.right.value: # Droite-Droite
        return rotate_left(node)
    if balance > 1 and value > node.left.value:   # Gauche-Droite
        node.left = rotate_left(node.left)
        return rotate_right(node)
    if balance < -1 and value < node.right.value: # Droite-Gauche
...
def to_dict_avl(node):
    ...
    return {
        "name": str(node.value),
        "children": [c for c in [to_dict_avl(node.left), to_dict_avl(node.right)] if c]
    }

@app.route("/avl/insert", methods=["POST"])
def insert_value_avl():
    global root_avl
    value = request.json["value"]
    root_avl = insert_avl(root_avl, value)
    return jsonify(to_dict_avl(root_avl))

@app.route("/avl/reset", methods=["POST"])
def reset_tree_avl():
    global root_avl
    root_avl = None
    return jsonify({"message": "AVL reset!"})
...
def tree_height_AVL(node):
    if node is None:
        return 0
    return 1 + max(tree_height_AVL(node.left), tree_height_AVL(node.right))

def node_count_AVL(node):
    if node is None:
        return 0
    return 1 + node_count_AVL(node.left) + node_count_AVL(node.right)
