...
const GrapheNonPondere = () => {
  ...
  const fetchGraph = async () => {
    const res = await fetch("http://localhost:5000/graphnonpondere/get");
  ...
  const addNode = async () => {
    await fetch("http://localhost:5000/graphnonpondere/add_node", {
      method: "POST",
  ...
  const addEdge = async () => {
    await fetch("http://localhost:5000/graphnonpondere/add_edge", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <NavBar />
      <HomePageOfTPOne />
      <h2 className="text-3xl font-bold mb-6">Graphe Non Pondéré</h2>
...
export default GrapheNonPondere;
