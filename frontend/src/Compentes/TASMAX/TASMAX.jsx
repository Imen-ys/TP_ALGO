...
const TASMAX = () => {
  ...
  const handleInsert = async () => {
    if (!number) return;
    const res = await fetch("http://127.0.0.1:5000/tasmax/insert", {
      method: "POST",
  ...
  const handleReset = async () => {
    await fetch("http://127.0.0.1:5000/tasmax/reset", { method: "POST" });
    setTreeData(null);
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
          <NavBar />
          <HomePageOfTPOne />
      <h1 className="text-3xl font-bold mb-6">Tas Max</h1>
