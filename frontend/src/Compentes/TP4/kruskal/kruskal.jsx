...
const Kruskal = () => {
  ...
  useEffect(() => {
    // Fetch the graph data when component mounts
    fetchGraphData();
  }, []);
  ...
  const fetchGraphData = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://127.0.0.1:5000/graphePondere/get");
  ...
  const executeKruskal = async () => {
    if (!graph) return alert("No graph available. Please upload a graph first.");
    
    try {
      setLoading(true);
      const response = await fetch("http://127.0.0.1:5000/kruskal/execute");
      const data = await response.json();
      setSteps(data.steps);
      setCurrentStep(0);
    } catch (error) {
      console.error("Error executing Kruskal's algorithm:", error);
      alert("Failed to execute Kruskal's algorithm");
    } finally {
      setLoading(false);
    }
  };
  ...
  const drawGraph = () => {
    ...
    const sortedEdges = currentStepData.sorted_edges || edges;
    ...
    edges.forEach(([from, to, weight]) => {
      ...
      if (isInTree) {
        ...
        ctx.strokeStyle = '#FF9800'; // Orange for currently considered edge
        ctx.lineWidth = 4;
      } else {
        ctx.strokeStyle = '#d0d0d0'; // Gray for other edges
        ctx.lineWidth = 2;
      }
  ...
  // Get steps to display (up to current step)
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="flex items-center space-x-4 mb-4">
              ...
              >
                {loading ? "Processing..." : "Run Kruskal's Algorithm"}
              </button>
              <button
